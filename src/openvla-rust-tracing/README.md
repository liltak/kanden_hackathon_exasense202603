...
> **注意**: 以下の画像はシステムの動作イメージです。実際の推論・追従結果とは異なります。

### 可視化の凡例

画像の種類によって色の意味が異なります。

**合成クラック画像（trajectory1, 2）― `infer.py` 出力（精度検証モード）**

| 色 | 意味 |
|----|------|
| 🟩 **緑矢印** | Ground Truth（正解軌跡）の移動方向 |
| 🟥 **赤矢印** | モデルの予測軌跡の移動方向 |

**実クラック画像（trajectory3, 4）― `rollout.py` 出力（ロールアウトモード）**

| 色 | 意味 |
|----|------|
| 🟩 **緑枠** | 追従開始パッチ（始点） |
| 🟦 **青枠** | 順番に観察した 224×224 パッチ列（中間ステップ） |
| 🟥 **赤枠** | 追従終了パッチ（終点） |
| ➡️ **シアン矢印** | モデルが予測した移動方向（パッチ間接続） |

### 合成クラック画像（自然画像背景 + クラック付与）

| trajectory1 | trajectory2 |
|:-----------:|:-----------:|
| ![trajectory1](assets/trajectory1_web.jpg) | ![trajectory2](assets/trajectory2_web.jpg) |

自然画像を背景として、プログラムで生成したクラックを合成した学習用データのサンプル。

### 実クラック画像

| trajectory3 | trajectory4 |
|:-----------:|:-----------:|
| ![trajectory3](assets/trajectory3_web.jpg) | ![trajectory4](assets/trajectory4_web.jpg) |

実際のコンクリート・石材等に発生したクラックの自然画像サンプル。
