"""
画像をクリックして起点を選び、座標を保存するスクリプト

使い方:
  # 単一画像
  python data_generation/pick_start.py \
...
def pick_with_opencv(image_path: str) -> tuple[int, int]:
    ...
    def on_mouse(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            cv2.imshow("クリックして起点を選択 (q で確定)", tmp)

    cv2.imshow("クリックして起点を選択 (q で確定)", disp)
    cv2.setMouseCallback("クリックして起点を選択 (q で確定)", on_mouse)
    print("画像をクリックして起点を選択してください。q で確定します。")
...
def main():
    parser = argparse.ArgumentParser(description="クリックで起点を選択して座標を保存")
