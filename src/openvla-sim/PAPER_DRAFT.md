# 論文ドラフト：OpenVLAによる日本語指示ドローンナビゲーション
...
**日本語：**
> GenesisシミュレータとOpenVLA 7Bを用いた自然言語指示によるドローンナビゲーション：自動データ収集とボディフレームアクション学習

**英語：**
> *Drone Navigation via Natural Language Instructions using OpenVLA 7B: Automated Data Collection with Body-Frame Action Representation in Genesis Simulator*
...
Genesis 物理シミュレータと PD コントローラを組み合わせた**完全自動データ収集パイプライン**により、人間デモを必要とせず訓練データを生成する。アクションは**機体座標系（ボディフレーム）**の4次元速度ベクトルとして表現し、ドローンの向きに依存しないフレーム不変な行動学習を実現する。また、日本語自然言語指示（「ソファに近づけ」等）による制御を実証し、英語に偏ったVLA研究における多言語対応の可能性を示す。
...
3. **物体指示ナビゲーションのVLA定式化：** 「\[物体名\] に近づけ」という自然言語指示から直接飛行制御アクションを生成するエンドツーエンドパイプライン
4. **日本語VLAの実証：** 日本語自然言語指示による制御の可能性と英語との比較
...
| 項目 | RaceVLA | **本研究** |
|------|---------|----------|
| タスク | ゲート通過レーシング | **室内物体への接近** |
| データ収集 | 人間デモ（推測） | **PIDによる完全自動収集** |
...
1. **collect.py：** Genesis シミュレータと PD コントローラによる自動データ収集
...
| Ablation 2 | 英語命令 vs 日本語命令 | 日本語対応の影響検証 |
...
| 提案手法（日本語・ボディフレーム） | \[TBD\] | \[TBD\] | \[TBD\] |
| Ablation: ワールドフレーム | \[TBD\] | \[TBD\] | \[TBD\] |
| Ablation: 英語命令 | \[TBD\] | \[TBD\] | \[TBD\] |
...
### 5.2 日本語VLAの可能性
- OpenVLA の Llama 2 バックボーンは英語中心に事前学習されているが、LoRA ファインチューニングにより日本語のパターン認識が可能
- 「ソファに近づけ」→「ソファまで飛べ」のような言い換えへの対応は英語より弱い可能性がある
- 実験での英日比較結果を踏まえた分析を記入予定
...
本研究では、Genesis シミュレータを用いた自動データ収集パイプラインと、ボディフレームアクション表現を組み合わせた OpenVLA ベースのドローンナビゲーション手法を提案した。日本語自然言語指示による室内ドローンの物体接近ナビゲーションを実証し、VLA モデルのロボットアーム操作以外への応用可能性を示した。
