...
| VGGT推論 (20枚) | **5.87秒** |
| 出力点群 | **3,480,960点** |
| SAM3前景抽出後 | **1,201,927点** (65%背景除去) |
| Peak VRAM | **11.19 GB** |
| DMTetメッシュ抽出 | **28秒** (grid=128) |
| メッシュ出力 | **20,000面** |
...
**SAM3 テキストプロンプト前景抽出** (新規開発)
- "building", "roof", "wall" で前景を指定
- "sky", "ground", "tree" で背景を除去
- パネル適合面 645 → **2,143面 (3.3倍)**
- SAM3 → SAM2 自動フォールバック対応

**Kaolin DMTet** (NKSR代替)
- 非商用ライセンスの NKSR を排除
- NVIDIA Kaolin の DMTet で GPU メッシュ再構成
- VRAM **1.4GB** (NKSRの1/20)、Apache 2.0

**Convex Hull プリミティブ近似** (新規開発)
- SAM3で地面除去 → デノイズ → Convex Hull
- 地面法線から建物の向きを自動補正
- 22面の軽量メッシュで安定した日照シミュレーション
...
### H100 学習実績
- **50エピソード / 587ステップ** の合成データ生成
- OpenVLA 7B + LoRA (rank=16) を H100 で **6分** で学習完了
- Train Loss: 0.44 → **0.15** (5 epoch)、Val Loss: **0.19**
- 評価: カバレッジ率 12.7% (3テスト画像)
...
### 技術的限界
- **メッシュ処理時間**: Poisson再構成が 319秒 → DMTetで **18秒** に改善済み
  - → Convex Hull プリミティブ近似でさらに高速化の余地
...
- **前景抽出は点群品質に直結** (SAM3導入でパネル適合面 3.3倍↑、NPV +43%)
